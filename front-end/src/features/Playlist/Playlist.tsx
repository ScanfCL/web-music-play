...
export const Playlist = () => {
  ...
  const { selectPlaylistId, setSelectPlaylistId } = usePlaylistContext();
  const { data: playlistMusics } = useGetPlaylistMusic({
    playlistId: selectPlaylistId,
  });
  const { handleSetMusics } = usePlayerContext();
  const { setIsSearching } = useSearchContext();

  const { mutate: deletePlaylist } = useMutation(
    ({ playlistId }: { playlistId: string }) =>
      playlistService.removePlaylist({ id: playlistId }),
    {
      onSuccess: () => {
  ...
  const musics = useMemo(
    () => playlistMusics?.map((pm) => pm.music),
    [playlistMusics],
  );
  ...
  const handleClickPlaylist = useCallback(
    ...
    [handleSetMusics, musics, setIsSearching, setSelectPlaylistId],
  );
      ...
      {data?.map((playlist) => (
        <div
          ...
          className={classNames(
            'h-[32px] flex items-center justify-between cursor-pointer my-[8px]',
            { 'text-[#b3b3b3]': playlist.id !== selectPlaylistId },
          )}
